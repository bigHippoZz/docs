    ...
    _prepareGroup = function (options) {
        function toFn(value, pull) {
            return function (to, from, dragEl, evt) {
                ...
                // 未设置 value 并且 pull 操作或者是同一个组
                if (value == null && (pull || sameGroup)) {
                    // Default pull value
                    // Default pull and put value if same group
                    return true
                    // 未设置put 或者为false 就不让操作
                    ...
                    return (
                        value === true /* 允许当前操作 不管是pull 还是put */ ||
                        (typeof value === 'string' &&
                            value ===
                                otherGroup) /* 判断是不是当前将要操作的容器 */ ||
                        (value.join &&
                            value.indexOf(otherGroup) >
                                -1) /* 判断当前操作的容器在不在数组中 */
                    )
                }
            }
        ...
        group.checkPull = toFn(originalGroup.pull /* 拉 */, true)
        group.checkPut = toFn(originalGroup.put /* 放 */)
...
function Sortable(el, options) {
    ...
    _prepareGroup(options) /* 准备拖拽到不同分组的操作 */
...
Sortable.prototype = /** @lends Sortable.prototype */ {
    ...
    _onTapStart: function (/** Event|TouchEvent */ evt) {
        ...
        let _this = this,
            ...
            type = evt.type /* 当前事件类型 */,
            touch =
                /* 猜测如果是touch事件 拿到touch事件event */
            ...
            if (parent) {
                do {
                    if (parent[expando]) {
                        ...
                        inserted = parent[expando]._onDragOver_onDragOver({
...
Sortable.mount = function (...plugins) {
    ...
    plugins.forEach(plugin => {
        ...
        if (plugin.utils)
            Sortable.utils = { ...Sortable.utils, ...plugin.utils }
